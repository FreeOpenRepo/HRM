...
import HangfireSchedulerView from '@/components/views/HangfireSchedulerView';
import { Users, UserCheck, CalendarClock, Wifi, WifiOff } from 'lucide-react';

export default function Home() {
  const [activeRole, setActiveRole] = useState<ActorRole>('Employee');
  ...
  async function checkHealth() {
    try {
      const res = await fetch('http://localhost:5060/api/health');
  ...
  return (
    <main style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      >
        <div style={{ maxWidth: '1500px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
          ...
          <div style={{
            ...
            {[
              { role: 'Employee' as const, label: 'Employee Portal', icon: UserCheck, color: 'var(--accent-emerald)' },
              { role: 'HRAdmin' as const, label: 'HR Admin & Payroll', icon: Users, color: 'var(--accent-cyan)' },
              { role: 'HangfireScheduler' as const, label: 'Hangfire Cron Desk', icon: CalendarClock, color: 'var(--accent-purple)' },
            ].map(tab => {
              const Icon = tab.icon;
          ...
          <div style={{
            ...
            <span>{isApiOnline ? 'HRM API ออนไลน์' : 'Connecting API :5060...'}</span>
          </div>
        </div>
      </header>
      ...
      <div style={{ flex: 1, padding: '16px' }}>
        {activeRole === 'Employee' && <EmployeeView />}
        {activeRole === 'HRAdmin' && <HRAdminView />}
        {activeRole === 'HangfireScheduler' && <HangfireSchedulerView />}
      </div>
    </main>
  );
}
